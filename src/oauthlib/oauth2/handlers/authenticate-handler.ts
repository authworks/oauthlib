...
export class AuthenticateHandler {
  ...
  async handle(request: Request, response: Response) {
    ...
    try {
      ...
      if (e instanceof UnauthorizedRequestError) {
        response.header('WWW-Authenticate', 'Bearer realm="Service"');
      }
  ...
  updateResponse(response: Response, accessToken: Token) {
    if (this.scope && this.addAcceptedScopesHeader) {
      response.header('X-Accepted-OAuth-Scopes', this.scope);
    }

    if (this.scope && this.addAuthorizedScopesHeader && accessToken.scope) {
      response.header('X-OAuth-Scopes', accessToken.scope);
    }
  }
}
